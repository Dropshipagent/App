...
if ($tab == "Approved_and_Unpaid") {
    ?>
    <!--<a href="{{ URL::to('admin/users/' . $user->id . '/edit') }}" class="btn btn-primary margin2px" title="Edit Product"><i class="fa fa-edit"></i></a>
    <a href="{{ url('admin/products/index', $user->username) }}" class="btn btn-success margin2px" title="Products"><i class="fa fa-product-hunt"></i></a>-->
    <a href="{{ URL::to('admin/users/profile/' . $user->id ) }}" class="btn btn-success margin2px" title="Profile"><i class="fa fa-user "></i></a>
...
if ($tab == "Approved_and_Paid") {
    ...
    if ($role == 2) {
        ...
        <?php if (!isset($user->get_supplier->store_id)) { ?>
            <a href="javascript:void(0);" class="btn btn-danger assign_supplier_btn margin2px" data-id="{{ $user->id }}" data-val="{{ $user->username }}" title="Assign Supplier"><i class="fa fa-bookmark"></i></a>
        <?php } ?>
        <!--<a href="{{ url('admin/products/index', $user->username) }}" class="btn btn-success margin2px" title="Products"><i class="fa fa-product-hunt"></i></a>
...
if ($tab == "Suppliers") {
    ?>
    <a href="javascript:void(0)" data-id="{{$user->id}}" class="btn btn-warning showMappedStoreList" title="View Store">View Store</a>
    <?php
    /*
      ?>
      <form action="{{ route('users.destroy', $user->id) }}" method="POST" style="display: inline" onsubmit="return confirm('Are you sure?');">
